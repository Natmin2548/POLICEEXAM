...
// --- Custom Room & Tournament APIs ---

// 1. Create Room (2-8 players)
app.post('/api/battle/room/create', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    const maxP = Math.min(Math.max(parseInt(maxPlayers) || 8, 2), 8);
    ...
    res.json({ success: true, roomCode: room.roomCode, room });
  } catch (err) {
    console.error('Create Room Error:', err);
    res.status(500).json({ error: 'เกิดข้อผิดพลาดในการสร้างห้องประลอง' });
  }
});

// 2. Get Room State
app.get('/api/battle/room/:code', authenticateToken, async (req, res) => {
  try {
    ...
    if (!room) {
      return res.status(404).json({ error: 'ไม่พบห้องประลองนี้' });
    }

    res.json(room);
  } catch (err) {
    console.error('Get Room Error:', err);
    res.status(500).json({ error: 'เกิดข้อผิดพลาดในการดึงข้อมูลห้องประลอง' });
  }
});
...
app.post('/api/battle/room/join', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ error: 'เกิดข้อผิดพลาดในการเข้าห้องประลอง' });
  }
});

// 4. Start Room Duel (Host starts or countdown finishes)
app.post('/api/battle/room/start', authenticateToken, async (req, res) => {
  const { roomCode, subject } = req.body;
  try {
    const room = await prisma.battleRoom.findUnique({
      where: { roomCode: (roomCode || '').toUpperCase() }
    });
    ...
    // Selected subject or random subject
    const selectedSubject = subject || 'ทั่วไป';

    // Fetch 10 random questions for this subject
    const sets = await prisma.examSet.findMany({
      where: { category: selectedSubject },
      select: { id: true }
    });
    ...
    res.status(500).json({ error: 'เกิดข้อผิดพลาดในการเริ่มประลอง' });
  }
});
