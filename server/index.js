...
function buildComputerPrompt({ count, subcategory, title, contextText }) {
  const target = `${subcategory || ''} ${title || ''}`.toLowerCase();
  const chMatch = target.match(/บทที่\s*(\d+)/);
  const chNum = chMatch ? parseInt(chMatch[1], 10) : 0;

  let chapterTitle = 'วิชาเทคโนโลยีสารสนเทศและคอมพิวเตอร์เพื่อการสื่อสาร';
  let chapterSpecificRules = '';

  if (chNum === 1 || target.includes('ความรู้พื้นฐานและประวัติ')) {
    chapterTitle = 'บทที่ 1: ความรู้พื้นฐานและประวัติคอมพิวเตอร์';
    chapterSpecificRules = `🎯 กฎเหล็กเฉพาะสำหรับ "บทที่ 1 ประวัติและวิวัฒนาการคอมพิวเตอร์":
- ยุคคอมพิวเตอร์ 5 ยุค: ยุคที่ 1 หลอดสุญญากาศ (Vacuum Tube), ยุคที่ 2 ทรานซิสเตอร์ (Transistor), ยุคที่ 3 วงจรรวม (IC - Integrated Circuit), ยุคที่ 4 วงจรรวมขนาดใหญ่มาก (VLSI / Microprocessor), ยุคที่ 5 ปัญญาประดิษฐ์ (AI)
- บิดาแห่งคอมพิวเตอร์: ชาร์ลส์ แบบเบจ (Charles Babbage - เครื่อง Difference Engine / Analytical Engine)
- โปรแกรมเมอร์คนแรกของโลก: เอดา เลิฟเลซ (Ada Lovelace)`;
  } else if (chNum === 2 || target.includes('ข้อมูลและสารสนเทศ')) {
    chapterTitle = 'บทที่ 2: ข้อมูลและสารสนเทศ';
    chapterSpecificRules = `🎯 กฎเหล็กเฉพาะสำหรับ "บทที่ 2 ข้อมูลและสารสนเทศ":
- ข้อมูล (Data - ข้อเท็จจริงดิบที่ยังไม่ได้ประมวลผล) vs สารสนเทศ (Information - ข้อมูลที่ผ่านการประมวลผล จัดหมวดหมู่ วิเคราะห์แล้วนำไปใช้ตัดสินใจได้)
- วงจรการประมวลผลข้อมูล (Data Processing Cycle): Input -> Processing -> Output -> Storage
- คุณสมบัติของสารสนเทศที่ดี: ถูกต้อง แม่นยำ ทันสมัย สมบูรณ์ ตรงตามความต้องการ`;
  } else if (chNum === 3 || target.includes('ipos') || target.includes('หน่วยประมวลผล')) {
    chapterTitle = 'บทที่ 3: IPOS และหน่วยประมวลผล';
    chapterSpecificRules = `🎯 กฎเหล็กเฉพาะสำหรับ "บทที่ 3 IPOS และฮาร์ดแวร์":
- หน่วยประมวลผลกลาง (CPU): หน่วยคำนวณและตรรกะ (ALU), หน่วยควบคุม (Control Unit: CU), รีจิสเตอร์ (Register)
- หน่วยความจำหลัก: RAM (Random Access Memory - ชั่วคราว/ลบเลือนได้ Volatile) vs ROM (Read-Only Memory - ถาวร/Non-volatile บันทึก BIOS/Firmware)
- หน่วยความจำแคช (Cache Memory): เร็วมาก อยู่ใกล้/ใน CPU เพื่อลดคอขวดระหว่าง CPU กับ RAM
- หน่วยเก็บข้อมูลสำรอง: HDD, SSD, Flash Drive, Optical Disc`;
  } else if (chNum === 4 || target.includes('ซอฟต์แวร์') || target.includes('software')) {
    chapterTitle = 'บทที่ 4: ซอฟต์แวร์ (Software)';
    chapterSpecificRules = `🎯 กฎเหล็กเฉพาะสำหรับ "บทที่ 4 ซอฟต์แวร์":
- ซอฟต์แวร์ระบบ (System Software): ระบบปฏิบัติการ (OS เช่น Windows, macOS, Linux, Android, iOS), โปรแกรมอรรถประโยชน์ (Utility Programs เช่น Antivirus, Disk Defragmenter), ดีไวซ์ไดรเวอร์ (Device Driver), ตัวแปลภาษา (Compiler, Interpreter)
- ซอฟต์แวร์ประยุกต์ (Application Software): งานทั่วไป (MS Office, Web Browser) vs งานเฉพาะด้าน (โปรแกรมบัญชี, ERP, ระบบสถิติคดีตำรวจ CRIMES)
- สิทธิ์การใช้งาน: Freeware, Shareware, Open Source (แก้ไขโค้ดได้), Commercial/Proprietary`;
  } else if (chNum === 5 || target.includes('รหัสแทนข้อมูล') || target.includes('ชนิดข้อมูล')) {
    chapterTitle = 'บทที่ 5: ชนิดข้อมูลและรหัสแทนข้อมูล';
    chapterSpecificRules = `🎯 กฎเหล็กเฉพาะสำหรับ "บทที่ 5 รหัสแทนข้อมูลและหน่วยวัด":
- หน่วยวัดข้อมูล: 1 Byte = 8 Bits, 1 KB = 1,024 Bytes, 1 MB = 1,024 KB, 1 GB = 1,024 MB, 1 TB = 1,024 GB
- รหัสแทนข้อมูล: ASCII (มาตรฐาน 7 บิต 128 ตัวอักขระ, Extended ASCII 8 บิต 256 ตัวอักขระ), Unicode (UTF-8, UTF-16 รองรับทุกภาษาทั่วโลกรวมถึงภาษาไทย)
- ระบบเลขฐาน: เลขฐานสอง (Binary: 0, 1), ฐานแปด (Octal: 0-7), ฐานสิบ (Decimal: 0-9), ฐานสิบหก (Hexadecimal: 0-9 และ A-F)`;
  } else if (chNum === 6 || target.includes('procedure') || target.includes('ผังงาน') || target.includes('flowchart')) {
    chapterTitle = 'บทที่ 6: Procedure และผังงาน (Flowchart)';
    chapterSpecificRules = `🎯 กฎเหล็กเฉพาะสำหรับ "บทที่ 6 ผังงาน (Flowchart) และอัลกอริทึม":
- สัญลักษณ์ Flowchart สากล:
  * วงรี / ขอบมน (Terminal): จุดเริ่มต้น (Start) หรือสิ้นสุด (End/Stop)
  * สี่เหลี่ยมผืนผ้า (Process): การประมวลผล คำนวณ หรือกำหนดค่า
  * สี่เหลี่ยมขนมเปียกปูน / สี่เหลี่ยมข้าวหลามตัด (Decision): การตัดสินใจ เงื่อนไข (มีทางเลือก Yes/No หรือ จริง/เท็จ)
  * สี่เหลี่ยมด้านขนาน (Input/Output): การรับข้อมูลหรือแสดงผลทั่วไปโดยไม่ระบุอุปกรณ์
  * วงกลม (Connector): จุดเชื่อมต่อภายในหน้าเดียวกัน
  * ห้าเหลี่ยม / สัญลักษณ์ Off-page: จุดเชื่อมต่อคนละหน้า
  * เส้นลูกศร (Flowline): ทิศทางการทำงาน`;
  } else if (chNum === 7 || target.includes('ระบบเครือข่าย') || target.includes('network')) {
    chapterTitle = 'บทที่ 7: ระบบเครือข่ายคอมพิวเตอร์ (Computer Network)';
    chapterSpecificRules = `🎯 กฎเหล็กเฉพาะสำหรับ "บทที่ 7 ระบบเครือข่าย":
- ประเภทเครือข่ายตามระยะทาง: PAN (ส่วนบุคคล Bluetooth), LAN (ท้องถิ่นในอาคาร/สำนักงาน), MAN (ระดับเมือง), WAN (วงกว้างระดับประเทศ/โลก เช่น อินเทอร์เน็ต)
- โครงสร้างเครือข่าย (Topology): Star (ต่อเข้า Switch ส่วนกลาง นิยมสุด), Bus (สายแกนร่วมเส้นเดียว), Ring (วงแหวน), Mesh (ร่างแห เชื่อมโยงทุกจุด ทนทานสุด)
- อุปกรณ์เครือข่าย: Switch (ส่งข้อมูลตาม MAC Address ไปเฉพาะพอร์ตปลายทาง), Router (เชื่อมต่อต่างเครือข่าย คัดเลือกเส้นทางตาม IP Address), Access Point, Gateway, Modem`;
  } else if (chNum === 8 || target.includes('internet') || target.includes('อินเทอร์เน็ต')) {
    chapterTitle = 'บทที่ 8: ระบบอินเทอร์เน็ตและโปรโตคอล (Internet & Protocols)';
    chapterSpecificRules = `🎯 กฎเหล็กเฉพาะสำหรับ "บทที่ 8 อินเทอร์เน็ตและโปรโตคอล":
- IP Address: IPv4 (32 บิต แบ่งเป็น 4 ชุด คั่นด้วยจุด เช่น 192.168.1.1) vs IPv6 (128 บิต แบ่งเป็น 8 ชุด คั่นด้วยโคลอน เช่น 2001:0db8::1)
- บริการเครือข่าย: DNS (Domain Name System - แปลงชื่อเว็บเป็น IP Address), DHCP (แจกจ่าย IP Address อัตโนมัติ)
- โครงสร้าง URL และโปรโตคอล: HTTP (พอร์ต 80), HTTPS (พอร์ต 443 เข้ารหัสความปลอดภัย SSL/TLS), FTP (พอร์ต 20/21 รับส่งไฟล์), SMTP (พอร์ต 25 ส่งอีเมล), POP3 (พอร์ต 110 ดึงอีเมล), IMAP (พอร์ต 143 ดึงอีเมลซิงค์หลายอุปกรณ์)`;
  } else if (chNum === 9 || target.includes('e-commerce') || target.includes('พาณิชย์อิเล็กทรอนิกส์')) {
    chapterTitle = 'บทที่ 9: E-commerce (พาณิชย์อิเล็กทรอนิกส์)';
    chapterSpecificRules = `🎯 กฎเหล็กเฉพาะสำหรับ "บทที่ 9 E-commerce":
- รูปแบบธุรกิจ: B2B (ธุรกิจกับธุรกิจ), B2C (ธุรกิจกับผู้บริโภค เช่น ร้านค้าออนไลน์ Shopee/Lazada), C2C (ผู้บริโภคกับผู้บริโภค เช่น ตลาดนัดมือสอง), G2C (รัฐกับประชาชน เช่น เสียภาษีออนไลน์ ชำระค่าปรับจราจร e-Ticket)
- ความปลอดภัยการชำระเงิน: OTP (One-Time Password), 2FA, Payment Gateway, e-Wallet, Digital Signature`;
  } else if (chNum === 10 || target.includes('ความปลอดภัย') || target.includes('security') || target.includes('ไวรัส') || target.includes('พ.ร.บ.คอม')) {
    chapterTitle = 'บทที่ 10: ความมั่นคงปลอดภัยไซเบอร์และ พ.ร.บ.คอมพิวเตอร์';
    chapterSpecificRules = `🎯 กฎเหล็กเฉพาะสำหรับ "บทที่ 10 ความปลอดภัยและ พ.ร.บ.คอมพิวเตอร์":
- ภัยคุกคามไซเบอร์: Phishing (หลอกลวงขโมยรหัสผ่านผ่านเว็บปลอม/ลิงก์ปลอม), Ransomware (มัลแวร์เรียกค่าไถ่ เข้ารหัสไฟล์ข้อมูล), Trojan Horse (ม้าโทรจัน แฝงตัวมากับโปรแกรมอื่น), Worm (หนอนคอมพิวเตอร์ แพร่กระจายตัวเองผ่านเน็ตไม่ต้องอาศัยไฟล์พาหะ), Spyware, DDoS
- มาตรการป้องกัน: Firewall, 2FA/MFA (การยืนยันตัวตนแบบหลายชั้น), Encryption (การเข้ารหัสข้อมูล)
- พ.ร.บ.ว่าด้วยการกระทำความผิดเกี่ยวกับคอมพิวเตอร์ พ.ศ. ๒๕๕๐ และแก้ไขเพิ่มเติม พ.ศ. ๒๕๖๐: เข้าถึงระบบโดยมิชอบ (ม.5), ดักรับข้อมูล (ม.8), แก้ไขดัดแปลงข้อมูล (ม.9), ทำลายระบบ/ส่งผลต่อความมั่นคง, ส่งสแปม/สแปมเมล (ม.11), นำเข้าข้อมูลเท็จ/ลามก/กระทบความมั่นคง (ม.14)`;
  } else if (chNum === 11 || target.includes('social media') || target.includes('cloud')) {
    chapterTitle = 'บทที่ 11: โซเชียลมีเดียและคลาวด์คอมพิวติ้ง (Social Media & Cloud)';
    chapterSpecificRules = `🎯 กฎเหล็กเฉพาะสำหรับ "บทที่ 11 โซเชียลมีเดียและ Cloud Computing":
- รูปแบบบริการ Cloud: IaaS (Infrastructure as a Service เช่น เครื่องเซิร์ฟเวอร์เสมือน AWS EC2, GCP Compute), PaaS (Platform as a Service สำหรับนักพัฒนา), SaaS (Software as a Service เช่น Google Workspace, MS 365, Canva)
- รูปแบบการติดตั้ง: Public Cloud, Private Cloud, Hybrid Cloud
- มารยาทและกฎหมายในโซเชียลมีเดีย: Cyberbullying, Fake News, ลิขสิทธิ์รูปภาพและเพลงบนสื่อออนไลน์`;
  } else if (chNum === 12 || target.includes('word') || target.includes('ประมวลผลคำ')) {
    chapterTitle = 'บทที่ 12: Microsoft Word (โปรแกรมประมวลผลคำ)';
    chapterSpecificRules = `🎯 กฎเหล็กเฉพาะสำหรับ "บทที่ 12 Microsoft Word":
- คีย์ลัดสำคัญ: Ctrl+N (เอกสารใหม่), Ctrl+O (เปิด), Ctrl+S (บันทึก), Ctrl+P (พิมพ์), Ctrl+Z (เลิกทำ Undo), Ctrl+Y (ทำซ้ำ Redo), Ctrl+B (ตัวหนา), Ctrl+I (ตัวเอียง), Ctrl+U (ขีดเส้นใต้), Ctrl+F (ค้นหา), Ctrl+H (แทนที่ Replace), Ctrl+A (เลือกทั้งหมด), Ctrl+Enter (ขึ้นหน้าใหม่ Page Break)
- ฟังก์ชันการทำงาน: การตั้งระยะขอบกระดาษ (Margins), สารบัญอัตโนมัติ (Table of Contents โดยใช้ Heading Styles), จดหมายเวียน (Mail Merge - รวมจดหมายหลักกับแหล่งข้อมูลรายชื่อ), การแบ่งส่วน (Section Break เพื่อใส่เลขหน้าต่างรูปแบบ)`;
  } else if (chNum === 13 || target.includes('excel') || target.includes('ตารางคำนวณ')) {
    chapterTitle = 'บทที่ 13: Microsoft Excel (โปรแกรมตารางคำนวณ)';
    chapterSpecificRules = `🎯 กฎเหล็กเฉพาะสำหรับ "บทที่ 13 Microsoft Excel":
- การอ้างอิงเซลล์: สัมพัทธ์ (Relative เช่น A1 เลื่อนตามสูตร) vs สัมบูรณ์ (Absolute ตรึงตำแหน่งด้วย $ เช่น $A$1 หรือ $A1, A$1) กดปุ่ม F4 เพื่อสลับ
- สูตรและฟังก์ชันคำนวณจริง:
  * SUM(range) รวมผลบวก, AVERAGE(range) ค่าเฉลี่ย, COUNT(range) นับเฉพาะตัวเลข, COUNTA(range) นับเซลล์ที่ไม่ว่าง, COUNTIF(range, criteria) นับตามเงื่อนไข
  * IF(logical_test, value_if_true, value_if_false) การทดสอบเงื่อนไข
  * VLOOKUP(lookup_value, table_array, col_index, [range_lookup]), XLOOKUP
  * MAX, MIN
- เครื่องหมายคำนวณ: บวก (+), ลบ (-), คูณ (*), หาร (/), ยกกำลัง (^), เชื่อมข้อความ (&)`;
  } else if (chNum === 14 || target.includes('powerpoint') || target.includes('นำเสนอ')) {
    chapterTitle = 'บทที่ 14: Microsoft PowerPoint (โปรแกรมนำเสนอข้อมูล)';
    chapterSpecificRules = `🎯 กฎเหล็กเฉพาะสำหรับ "บทที่ 14 Microsoft PowerPoint":
- คีย์ลัดสำคัญ: F5 (เริ่มฉายสไลด์ตั้งแต่ต้น), Shift+F5 (ฉายสไลด์ปัจจุบันที่กำลังเลือกอยู่), Esc (ยกเลิกการฉายสไลด์), B (หน้าจอดำ Black screen), W (หน้าจอขาว White screen), Ctrl+M (เพิ่มสไลด์ใหม่)
- ฟังก์ชันสำคัญ:
  * Slide Master (ต้นแบบสไลด์ - แก้ไขรูปแบบธีม ฟอนต์ โลโก้ ให้มีผลกับทุกสไลด์พร้อมกัน)
  * Transition (การเปลี่ยนผ่านระหว่างหน้าสไลด์) vs Animation (การเคลื่อนไหวของวัตถุ/ข้อความภายในสไลด์)`;
  } else {
    chapterTitle = subcategory || 'เทคโนโลยีสารสนเทศและคอมพิวเตอร์';
    chapterSpecificRules = `🎯 กฎเหล็กโครงสร้างข้อสอบคอมพิวเตอร์ตำรวจ:
- ระบบปฏิบัติการและคีย์ลัด, โปรแกรมสำนักงาน (Word, Excel, PowerPoint), เครือข่ายและอินเทอร์เน็ต, ความปลอดภัยไซเบอร์และ พ.ร.บ.คอมพิวเตอร์`;
  }
...
⛔️ 🇹🇭 กฎเหล็กภาษาไทย 100% (STRICT THAI LANGUAGE ONLY - บังคับ 100%):
1. **ข้อสอบวิชานี้เป็นข้อสอบภาษาไทยทั้งหมด 100%**
2. **ทั้งข้อความโจทย์คำถาม (questionText), ตัวเลือกทุกข้อ (optionA, optionB, optionC, optionD), และคำอธิบายเฉลย (explanation) ต้องเขียนเป็นภาษาไทย 100% เท่านั้น!**
3. ❌ **ห้ามออกข้อสอบเป็นภาษาอังกฤษทั้งประโยคเด็ดขาด!**
4. หากมีชื่อโปรแกรม คีย์ลัด หรือศัพท์เทคนิค ให้เขียนชื่อสากลพร้อมคำแปลหรือคำอธิบายภาษาไทยกำกับ เช่น "Ctrl + S", "Microsoft Word", "ซอฟต์แวร์ประมวลผลคำ (Word Processing Software)"
5. ❌ **ห้ามสร้างตัวเลือกห้วนๆ** เช่น "นำเสนอ", "คำนวณ" โดดๆ ตัวเลือกต้องเป็นประโยคหรือข้อความที่สมบูรณ์ชัดเจน
...
[
  {
    "questionText": "โจทย์คำถามเกี่ยวกับคอมพิวเตอร์และสารสนเทศที่เป็นภาษาไทย...",
    "optionA": "ตัวเลือก ก (ภาษาไทย)",
    "optionB": "ตัวเลือก ข (ภาษาไทย)",
    "optionC": "ตัวเลือก ค (ภาษาไทย)",
    "optionD": "ตัวเลือก ง (ภาษาไทย)",
    "correctOption": "A",
    "explanation": "คำอธิบายเฉลยอย่างละเอียดเป็นภาษาไทย อธิบายการทำงานของระบบและชี้จุดผิดของตัวเลือกอื่น..."
  }
]`;
}
...
function buildLawPrompt({ count, subcategory, title, contextText }) {
  ...
  let chapterTitle = 'กฎหมายที่ประชาชนควรรู้และกฎหมายตำรวจ';
  let chapterSpecificRules = '';

  if (chNum === 11 || target.includes('มรดก') || target.includes('พินัยกรรม')) {
    chapterTitle = 'บทที่ 11: กฎหมายแพ่ง — มรดกและพินัยกรรม';
    chapterSpecificRules = `🎯 กฎเหล็กเฉพาะสำหรับ "บทที่ 11 มรดกและพินัยกรรม" (บังคับ 100%):
1. **ทุกข้อ (100%) ต้องออกเกี่ยวกับกองมรดก ทายาทโดยธรรม และพินัยกรรมเท่านั้น (ห้ามออกกฎหมายอาญาเด็ดขาด!):**
   - **กองมรดกและความรับผิด:** กองมรดกได้แก่ทรัพย์สินทุกชนิดของผู้ตาย สิทธิ หน้าที่ และความรับผิดต่างๆ (แต่ทายาทรับผิดไม่เกินกว่าทรัพย์มรดกที่ตนได้รับ)
   - **ลำดับทายาทโดยธรรม 6 ลำดับ:**
     ...
     5. ปู่ ย่า ตา ยาย
     6. ลุง ป้า น้า อา
     *หลักทายาทลำดับก่อนตัดทายาทลำดับหลัง*: ยกเว้น ลำดับที่ 1 (ผู้สืบสันดาน) และลำดับที่ 2 (บิดามารดา) ไม่ตัดกัน และมีสิทธิได้รับส่วนแบ่งร่วมกัน
   - **คู่สมรสที่ยังมีชีวิตอยู่:** เป็นทายาทโดยธรรมพิเศษ มีสิทธิได้รับส่วนแบ่งมรดกเสมือนเป็นทายาทชั้นผู้สืบสันดาน (หากมีลูก จะได้ส่วนแบ่งเท่ากับลูก 1 คน)
   - **คุณสมบัติผู้ทำพินัยกรรม:** ต้องมีอายุครบ 15 ปีบริบูรณ์ขึ้นไป และไม่เป็นบุคคลไร้ความสามารถ (หากอายุต่ำกว่า 15 ปีบริบูรณ์ พินัยกรรมตกเป็น "โมฆะ" ทันที)
   - **แบบของพินัยกรรม 5 แบบ:**
     1. แบบธรรมดา: ทำเป็นหนังสือ ลงวันเดือนปี มีพยานรู้เห็นพร้อมกันอย่างน้อย 2 คน
     2. แบบเขียนเองทั้งฉบับ: ผู้ทำพินัยกรรมต้องเขียนด้วยลายมือตนเองทั้งฉบับ ลงวันเดือนปีและลายมือชื่อ (ไม่ต้องมีพยาน)
     ...
     5. แบบทำด้วยวาจา: ใช้เฉพาะกรณีฉุกเฉินใกล้ตาย บอกข้อความต่อหน้าพยาน 2 คน แล้วพยานรีบไปแจ้งนายอำเภอโดยพลัน
   - **ข้อห้ามผู้รับมรดกตามพินัยกรรม:** พยานในพินัยกรรม หรือคู่สมรสของพยาน จะเป็นผู้รับทรัพย์ตามพินัยกรรมนั้นไม่ได้`;
  } else if (chNum === 1 || target.includes('ความรู้ทั่วไปเกี่ยวกับกฎหมาย')) {
    chapterTitle = 'บทที่ 1: ความรู้ทั่วไปเกี่ยวกับกฎหมาย';
    chapterSpecificRules = `🎯 กฎเหล็กเฉพาะสำหรับ "บทที่ 1 ความรู้ทั่วไปเกี่ยวกับกฎหมาย":
- บิดาแห่งกฎหมายไทย: พระเจ้าบรมวงศ์เธอ พระองค์เจ้ารพีพัฒนศักดิ์ กรมหลวงราชบุรีดิเรกฤทธิ์ (วันที่ 7 ส.ค. วันรพี)
- ลักษณะของกฎหมาย: มาจากรัฏฐาธิปัตย์, บังคับเป็นการทั่วไป, บังคับเสมอไป, มีสภาพบังคับ (Sanction)
- ลำดับศักดิ์ของกฎหมาย: รัฐธรรมนูญ -> พ.ร.ป. (10 ฉบับ) -> พ.ร.บ. / ประมวลกฎหมาย / พ.ร.ก. -> พระราชกฤษฎีกา -> กฎกระทรวง -> ประกาศกระทรวง -> ข้อบัญญัติท้องถิ่น
- กฎหมายสารบัญญัติ (กำหนดสิทธิหน้าที่) vs กฎหมายวิธีสบัญญัติ (กระบวนการดำเนินคดี)`;
  } else if (chNum === 2 || target.includes('ความรู้ทั่วไปเกี่ยวกับรัฐ')) {
    chapterTitle = 'บทที่ 2: ความรู้ทั่วไปเกี่ยวกับรัฐ';
    chapterSpecificRules = `🎯 กฎเหล็กเฉพาะสำหรับ "บทที่ 2 ความรู้ทั่วไปเกี่ยวกับรัฐ":
- องค์ประกอบของรัฐ 4 ประการ: ดินแดน, ประชากร, รัฐบาล, อำนาจอธิปไตย (สำคัญที่สุด)
- รูปแบบรัฐ: รัฐเดี่ยว (ไทย, อังกฤษ, ญี่ปุ่น) vs สหพันธรัฐ (สหรัฐฯ, มาเลเซีย, สวิตเซอร์แลนด์)
- เขตทางทะเลตาม UNCLOS: ทะเลอาณาเขต (ไม่เกิน 12 ไมล์ทะเล), เขตต่อเนื่อง (ไม่เกิน 24 ไมล์ทะเล), เขตเศรษฐกิจจำเพาะ EEZ (ไม่เกิน 200 ไมล์ทะเล), ทะเลหลวง`;
  } else if (chNum === 3 || target.includes('ประวัติศาสตร์กฎหมายไทย')) {
    chapterTitle = 'บทที่ 3: ประวัติศาสตร์กฎหมายไทย';
    chapterSpecificRules = `🎯 กฎเหล็กเฉพาะสำหรับ "บทที่ 3 ประวัติศาสตร์กฎหมายไทย":
- สมัยสุโขทัย: ปิตุราชา/พ่อปกครองลูก, ศิลาจารึกหลักที่ 1 (มรดก ค้าเสรี ร้องทุกข์สั่นกระดิ่ง)
- สมัยอยุธยา: พระธรรมศาสตร์ (มอญ/อินเดีย), เทวราชา, ศักดินา
- รัตนโกสินทร์: กฎหมายตราสามดวง (ร.1: ราชสีห์-มหาดไทย, คชสีห์-กลาโหม, บัวแก้ว-พระคลัง), กฎหมายลักษณะอาญา ร.ศ. 127 (ร.5-ร.6 ประมวลฉบับแรก), พ.ศ. 2481 ได้รับเอกราชทางการศาลสมบูรณ์`;
  } else if (chNum === 4 || target.includes('รัฐธรรมนูญ')) {
    chapterTitle = 'บทที่ 4: รัฐธรรมนูญ (กฎหมายสูงสุด)';
    chapterSpecificRules = `🎯 กฎเหล็กเฉพาะสำหรับ "บทที่ 4 รัฐธรรมนูญ":
- รัฐธรรมนูญ 2560 (ฉบับที่ 20): สภาคู่ สส. 500 คน (เขต 400 + บัญชีรายชื่อ 100 วาระ 4 ปี อายุ 25 ปี), สว. 200 คน (วาระ 5 ปี อายุ 40 ปี)
- ครม.: นายกฯ 1 คน + รัฐมนตรีไม่เกิน 35 คน (รวมไม่เกิน 36 คน) นายกฯ ดำรงตำแหน่งรวมเกิน 8 ปีไม่ได้
- องค์กรอิสระ 5 องค์กร (กกต. 7 คน, ป.ป.ช. 9 คน, คตง. 7 คน, ผู้ตรวจการแผ่นดิน 3 คน, กสม. 7 คน วาระ 7 ปี วาระเดียว)
- ผู้มีสิทธิเลือกตั้ง: สัญชาติไทย (แปลงสัญชาติต้องได้ 5 ปีขึ้นไป), อายุไม่ต่ำกว่า 18 ปีในวันเลือกตั้ง, มีชื่อในทะเบียนบ้านไม่น้อยกว่า 90 วัน`;
  } else if (chNum === 5 || target.includes('กฎหมายปกครอง')) {
    chapterTitle = 'บทที่ 5: กฎหมายปกครอง (กฎหมายมหาชน)';
    chapterSpecificRules = `🎯 กฎเหล็กเฉพาะสำหรับ "บทที่ 5 กฎหมายปกครอง":
- หลักการจัดระเบียบบริหารราชการแผ่นดิน: รวมอำนาจ (ส่วนกลาง: สำนักนายกฯ กระทรวง ทบวง กรม ตร.), แบ่งอำนาจ (ส่วนภูมิภาค: จังหวัด อำเภอ), กระจายอำนาจ (ส่วนท้องถิ่น: อบจ. เทศบาล อบต. กทม. พัทยา)
- นิติกรรมทางปกครอง: กฎ (มีผลทั่วไป), คำสั่งทางปกครอง (เจาะจงบุคคล เช่น คำสั่งลงโทษวินัย คำสั่งพักใบอนุญาต), สัญญาทางปกครอง
- ปฏิบัติการทางปกครอง (การกระทำกายภาพ ไม่มุ่งผลกฎหมาย เช่น กวาดถนน ขับรถดับเพลิง)`;
  } else if (chNum === 6 || target.includes('กฎหมายแพ่ง — บุคคล') || target.includes('สภาพบุคคล')) {
    chapterTitle = 'บทที่ 6: กฎหมายแพ่ง — บุคคล';
    chapterSpecificRules = `🎯 กฎเหล็กเฉพาะสำหรับ "บทที่ 6 บุคคล":
- สภาพบุคคล (ม.15): เริ่มต้นเมื่อ "คลอดและอยู่รอดเป็นทารก" สิ้นสุดลงเมื่อ "ตาย"
- การสาบสูญ: กรณีทั่วไปหายไป 5 ปี, กรณีพิเศษ (สงคราม ยานพาหนะอับปาง ภัยพิบัติอันตรายถึงชีวิต) หายไป 2 ปี ศาลสั่งให้เป็นคนสาบสูญ (ถือว่าตาย)
- ผู้เยาว์ (อายุไม่ครบ 20 ปีบริบูรณ์): บรรลุนิติภาวะเมื่ออายุครบ 20 ปี หรือเมื่อสมรสถูกต้อง (18 ปีขึ้นไป) นิติกรรมที่ทำโดยไม่ได้รับความยินยอมตกเป็น "โมฆียะ"
- คนเสมือนไร้ความสามารถ: อยู่ในความดูแลของ "ผู้พิทักษ์", คนไร้ความสามารถ: อยู่ในความดูแลของ "ผู้อนุบาล" นิติกรรมที่ทำตกเป็นโมฆียะทุกกรณี`;
  } else if (chNum === 7 || target.includes('กฎหมายแพ่ง — ทรัพย์') || target.includes('ทรัพย์และทรัพย์สิน')) {
    chapterTitle = 'บทที่ 7: กฎหมายแพ่ง — ทรัพย์และทรัพย์สิน';
    chapterSpecificRules = `🎯 กฎเหล็กเฉพาะสำหรับ "บทที่ 7 ทรัพย์":
- อสังหาริมทรัพย์ vs สังหาริมทรัพย์
- สังหาริมทรัพย์ชนิดพิเศษ: เรือกำปั่นหรือเรือมีระวาง 5 ตันขึ้นไป, แพ, สัตว์พาหนะ (ช้าง ม้า โค กระบือ ล่อ ลา ที่ทำตั๋วรูปพรรณแล้ว)
- ครอบครองปรปักษ์ (ม.1382): สงบ เปิดเผย เจตนาเป็นเจ้าของ อสังหาริมทรัพย์ 10 ปี, สังหาริมทรัพย์ 5 ปี
- ส่วนควบ (เจ้าของทรัพย์ประธานเป็นเจ้าของ), อุปกรณ์ (ของบริวาร), ดอกผลธรรมดา vs ดอกผลนิตินัย (ดอกเบี้ย ค่าเช่า เงินปันผล)`;
  } else if (chNum === 8 || target.includes('นิติกรรมและสัญญา')) {
    chapterTitle = 'บทที่ 8: กฎหมายแพ่ง — นิติกรรมและสัญญา';
    chapterSpecificRules = `🎯 กฎเหล็กเฉพาะสำหรับ "บทที่ 8 นิติกรรมและสัญญา":
- โมฆะ (Void): เสียเปล่ามาแต่แรกเริ่ม (วัตถุประสงค์ต้องห้ามชัดแจ้ง ขัดความสงบ ไม่ทำตามแบบ สำคัญผิดในสาระสำคัญ)
- โมฆียะ (Voidable): สมบูรณ์จนกว่าจะถูกบอกล้าง (ข่มขู่ กลฉ้อฉล สำคัญผิดในคุณสมบัติ) อายุความบอกล้าง 1 ปีนับแต่รู้ หรือไม่เกิน 10 ปีนับแต่ทำนิติกรรม
- สัญญาซื้อขายอสังหาฯ/สังหาฯ พิเศษ: ต้องทำเป็นหนังสือและจดทะเบียนต่อพนักงานเจ้าหน้าที่ มิฉะนั้นเป็นโมฆะ
- สัญญาเช่าอสังหาฯ เกิน 3 ปี: ต้องทำเป็นหนังสือและจดทะเบียน มิฉะนั้นฟ้องร้องได้เพียง 3 ปี (เช่าได้สูงสุดไม่เกิน 30 ปี)
- ดอกเบี้ยเงินกู้ยืม: กฎหมายห้ามคิดเกินร้อยละ 15 ต่อปี (หากไม่กำหนดไว้คิดร้อยละ 3 ต่อปี, ผิดนัดร้อยละ 5 ต่อปี)`;
  } else if (chNum === 9 || target.includes('กฎหมายแพ่ง — หนี้')) {
    chapterTitle = 'บทที่ 9: กฎหมายแพ่ง — หนี้';
    chapterSpecificRules = `🎯 กฎเหล็กเฉพาะสำหรับ "บทที่ 9 หนี้":
- วัตถุแห่งหนี้ 3 ประการ: การกระทำ, การงดเว้นกระทำการ, การส่งมอบทรัพย์สิน
- บ่อเกิดแห่งหนี้: นิติกรรมสัญญา (ความสมัครใจ) vs นิติเหตุ (กฎหมายกำหนด เช่น ละเมิด ลาภมิควรได้ จัดการงานนอกสั่ง)
- ความระงับแห่งหนี้ 5 ประการ: 1.การชำระหนี้ 2.การปลดหนี้ 3.การหักกลบลบหนี้ 4.การแปลงหนี้ใหม่ (Novation) 5.หนี้เกลื่อนกลืนกัน`;
  } else if (chNum === 10 || target.includes('กฎหมายแพ่ง — ครอบครัว')) {
    chapterTitle = 'บทที่ 10: กฎหมายแพ่ง — ครอบครัว';
    chapterSpecificRules = `🎯 กฎเหล็กเฉพาะสำหรับ "บทที่ 10 ครอบครัว":
- เงื่อนไขการหมั้นและการสมรส: ชายและหญิง (หรือคู่สมรส) ต้องมีอายุครบ 18 ปีบริบูรณ์ขึ้นไป หากฝ่าฝืนตกเป็น "โมฆะ" (ผู้เยาว์ต้องได้รับความยินยอม มิฉะนั้นเป็นโมฆียะ)
- สินส่วนตัว (มีอยู่ก่อนสมรส, เครื่องใช้ส่วนตัว, ของหมั้น, มรดกที่ระบุเป็นสินส่วนตัว) vs สินสมรส (ได้มาระหว่างสมรส, ดอกผลของสินส่วนตัว)
- บุตรชอบด้วยกฎหมาย: บุตรนอกสมรสเป็นบุตรชอบด้วยกฎหมายของมารดาเสมอ จะชอบด้วยกฎหมายของบิดาเมื่อ: จดทะเบียนสมรสภายหลัง, บิดาจดทะเบียนรับรองบุตร, ศาลพิพากษา
- บุตรบุญธรรม: ผู้รับต้องอายุไม่ต่ำกว่า 25 ปี และแก่กว่าบุตรบุญธรรมอย่างน้อย 15 ปี`;
  } else if (chNum >= 12 && chNum <= 15 || (target.includes('อาญา') && !target.includes('ทรัพย์'))) {
    chapterTitle = `บทที่ ${chNum || 12}: กฎหมายอาญา`;
    chapterSpecificRules = `🎯 กฎเหล็กเฉพาะสำหรับ "กฎหมายอาญา":
- หลักการ: ไม่มีกฎหมาย ไม่มีความผิด ไม่มีโทษ, กฎหมายอาญาไม่ย้อนหลังเป็นโทษ
- ลหุโทษ: จำคุกไม่เกิน 1 เดือน หรือปรับไม่เกิน 10,000 บาท หรือทั้งจำทั้งปรับ (ไม่ต้องมีเจตนาก็ผิด)
- โครงสร้างความรับผิด: องค์ประกอบภายนอก (ผู้กระทำ การกระทำ/งดเว้น วัตถุ), องค์ประกอบภายใน (เจตนา ประมาท เจตนาพลาด ม.60)
- พยายามกระทำความผิด (ม.80): ลงมือแล้วแต่ไม่บรรลุผล รับโทษ 2 ใน 3
- เหตุยกเว้นความผิด: ป้องกันโดยชอบ ม.68, เหตุยกเว้นโทษ: จำเป็น ม.67, บันดาลโทสะ ม.72 (ลดโทษ)
- ผู้เกี่ยวข้อง: ตัวการ ม.83 (รับโทษเต็ม), ผู้ใช้ ม.84 (รับโทษเสมือนตัวการ), ผู้สนับสนุน ม.86 (รับโทษ 2 ใน 3)`;
  } else if (chNum === 16 || target.includes('ความผิดเกี่ยวกับทรัพย์')) {
    chapterTitle = 'บทที่ 16: ความผิดเกี่ยวกับทรัพย์ (อาญา)';
    chapterSpecificRules = `🎯 กฎเหล็กเฉพาะสำหรับ "ความผิดเกี่ยวกับทรัพย์":
- ลักทรัพย์: เอาทรัพย์ของผู้อื่นไปโดยทุจริต
- วิ่งราวทรัพย์: ลักทรัพย์โดยฉกฉวยเอาซึ่งหน้า
- กรรโชกทรัพย์: ขู่เข็ญว่าจะใช้กำลังประทุษร้ายเพื่อให้ได้ทรัพย์
- รีดเอาทรัพย์: ขู่ว่าจะเปิดเผยความลับเพื่อให้ได้ทรัพย์
- ชิงทรัพย์: ลักทรัพย์โดยใช้กำลังประทุษร้ายหรือขู่เข็ญว่าในทันใดนั้นจะใช้กำลังประทุษร้าย
- ปล้นทรัพย์: ชิงทรัพย์โดยร่วมกันกระทำความผิดตั้งแต่ 3 คนขึ้นไป
- ยักยอก: ครอบครองทรัพย์ของผู้อื่นแล้วเบียดบังเอาเป็นของตน
- ฉ้อโกง: หลอกลวงด้วยการแสดงข้อความเท็จจนได้ทรัพย์สินไป
- รับของโจร: ช่วยซ่อนเร้น จำหน่าย พาเอาไปเสีย รับซื้อ รับจำนำ ซึ่งทรัพย์อันได้มาโดยความผิดฐานลักทรัพย์ วิ่งราวทรัพย์ กรรโชก รีดเอาทรัพย์ ชิงทรัพย์ ปล้นทรัพย์ ยักยอก ฉ้อโกง`;
  } else if (chNum === 17 || target.includes('ทรัพย์สินทางปัญญา')) {
    chapterTitle = 'บทที่ 17: ทรัพย์สินทางปัญญา';
    chapterSpecificRules = `🎯 กฎเหล็กเฉพาะสำหรับ "ทรัพย์สินทางปัญญา":
- ลิขสิทธิ์ (Copyright): เกิดขึ้นทันทีที่สร้างสรรค์ผลงาน ได้รับความคุ้มครองตลอดอายุผู้สร้างสรรค์ + อีก 50 ปีหลังเสียชีวิต
- สิทธิบัตร (Patent): ต้องจดทะเบียน สิทธิบัตรการประดิษฐ์คุ้มครอง 20 ปี, อนุสิทธิบัตรคุ้มครอง 6 ปี (ต่อได้ 2 ครั้ง ครั้งละ 2 ปี รวม 10 ปี), สิทธิบัตรการออกแบบ 10 ปี
- เครื่องหมายการค้า (Trademark): คุ้มครอง 10 ปีนับแต่วันยื่นคำขอ ต่ออายุได้คราวละ 10 ปี`;
  } else if (chNum === 18 || target.includes('pdpa') || target.includes('คุ้มครองผู้บริโภค')) {
    chapterTitle = 'บทที่ 18: กฎหมายคุ้มครองผู้บริโภคและ PDPA';
    chapterSpecificRules = `🎯 กฎเหล็กเฉพาะสำหรับ "คุ้มครองผู้บริโภคและ PDPA":
- สิทธิผู้บริโภค 5 ประการตาม พ.ร.บ.คุ้มครองผู้บริโภค (สิทธิได้รับข่าวสาร, อิสระในการเลือก, ได้รับความปลอดภัย, ได้รับความเป็นธรรมในสัญญา, ได้รับการชดเชยความเสียหาย)
- PDPA พ.ร.บ.คุ้มครองข้อมูลส่วนบุคคล พ.ศ. ๒๕๖๒: ข้อมูลส่วนบุคคลทั่วไป vs ข้อมูลส่วนบุคคลอ่อนไหว (Sensitive Data: เชื้อชาติ ศาสนา การเมือง ข้อมูลพันธุกรรม ข้อมูลสุขภาพ ประวัติอาชญากรรม)
- ฐานความยินยอม (Consent) และสิทธิของเจ้าของข้อมูล (สิทธิขอเข้าถึง สิทธิขอให้ลบ สิทธิคัดค้าน)`;
  } else if (chNum === 19 || target.includes('แรงงาน')) {
    chapterTitle = 'บทที่ 19: กฎหมายแรงงาน';
    chapterSpecificRules = `🎯 กฎเหล็กเฉพาะสำหรับ "กฎหมายแรงงาน":
- เวลาทำงานปกติ: ไม่เกิน 8 ชั่วโมงต่อวัน หรือ 48 ชั่วโมงต่อสัปดาห์ (งานอันตรายไม่เกิน 7 ชม./วัน หรือ 42 ชม./สัปดาห์)
- ค่าล่วงเวลา (OT): วันทำงานปกติ 1.5 เท่า, วันหยุดไม่น้อยกว่า 3 เท่า
- วันลา: ลาป่วยได้เท่าที่ป่วยจริง ลาตั้งแต่ 3 วันทำงานขึ้นไปนายจ้างอาจให้แสดงใบรับรองแพทย์ โดยได้รับค่าจ้างปีละไม่เกิน 30 วันทำงาน, ลาพักร้อนประจำปีไม่น้อยกว่า 6 วันทำงาน (ทำงานครบ 1 ปี)`;
  } else if (chNum === 20 || target.includes('ภาษี')) {
    chapterTitle = 'บทที่ 20: กฎหมายภาษีอากร';
    chapterSpecificRules = `🎯 กฎเหล็กเฉพาะสำหรับ "กฎหมายภาษีอากร":
- ภาษีทางตรง (ผู้เสียภาษีผลักภาระไม่ได้ เช่น ภาษีเงินได้บุคคลธรรมดา อัตราก้าวหน้า 5-35%, ภาษีเงินได้นิติบุคคล, ภาษีที่ดินและสิ่งปลูกสร้าง)
- ภาษีทางอ้อม (ผู้เสียภาษีผลักภาระให้ผู้บริโภคได้ เช่น ภาษีมูลค่าเพิ่ม VAT 7%, ภาษีสรรพสามิต, อากรแสตมป์, ภาษีศุลกากร)`;
  } else if (chNum === 21 || target.includes('ทะเบียนราษฎร์') || target.includes('บัตรประชาชน')) {
    chapterTitle = 'บทที่ 21: ทะเบียนราษฎร์และสิทธิพลเมือง';
    chapterSpecificRules = `🎯 กฎเหล็กเฉพาะสำหรับ "ทะเบียนราษฎร์และบัตรประชาชน":
- แจ้งเกิด: ภายใน 15 วันนับแต่วันเกิด
- แจ้งตาย: ภายใน 24 ชั่วโมงนับแต่เวลาตาย (หรือพบศพ)
- แจ้งย้ายที่อยู่ (ย้ายเข้า/ย้ายออก): ภายใน 15 วัน
- บัตรประจำตัวประชาชน: ผู้มีสัญชาติไทยอายุตั้งแต่ 7 ปีบริบูรณ์ขึ้นไปแต่ไม่เกิน 70 ปี ต้องทำบัตรภายใน 60 วัน บัตรมีอายุ 8 ปี`;
  } else if (chNum === 22 || target.includes('ตำรวจแห่งชาติ') || target.includes('พ.ร.บ.ตำรวจ')) {
    chapterTitle = 'บทที่ 22: พ.ร.บ.ตำรวจแห่งชาติ พ.ศ. ๒๕๖๕ และวินัยตำรวจ';
    chapterSpecificRules = `🎯 กฎเหล็กเฉพาะสำหรับ "พ.ร.บ.ตำรวจแห่งชาติ ๒๕๖๕":
- โครงสร้าง: ก.ตร. (คณะกรรมการข้าราชการตำรวจ ดูแลการบริหารงานบุคคล), ก.ต.ช. (คณะกรรมการนโยบายตำรวจแห่งชาติ ดูแลนโยบายยุทธศาสตร์), ก.พ.ค.ตร. (คณะกรรมการพิทักษ์ระบบคุณธรรมตำรวจ ร้องทุกข์อุทธรณ์)
- โทษทางวินัยข้าราชการตำรวจ 5 สถาน:
  * วินัยไม่ร้ายแรง: ภาคทัณฑ์, ทัณฑกรรม, กักยาม, กักขัง
  * วินัยร้ายแรง: ปลดออก, ไล่ออก (ปลดออกยังมีสิทธิได้รับบำเหน็จบำนาญเสมือนลาออก ส่วนไล่ออกไม่มีสิทธิ)`;
  } else {
    chapterTitle = subcategory || 'กฎหมายที่ประชาชนควรรู้';
    chapterSpecificRules = `🎯 กฎเหล็กเฉพาะสำหรับวิชากฎหมาย:
- ออกข้อสอบตรงตามหัวข้อ "${subcategory || title}" 100%
- ใช้เคสสถานการณ์จำลอง 60% + ตัวบทกฎหมายแม่นยำ 40%
- คำอธิบายเฉลยต้องอ้างอิงเลขมาตรา และวิเคราะห์ปรับบทกฎหมายเข้ากับข้อเท็จจริงในโจทย์`;
  }
...
⛔️ 🇹🇭 กฎเหล็กภาษาไทย 100% (STRICT THAI LANGUAGE ONLY - บังคับ 100%):
1. **ข้อสอบวิชานี้เป็น "กฎหมายไทย" ทั้งหมด 100%**
2. **ทั้งข้อความโจทย์คำถาม (questionText), ตัวเลือกทุกข้อ (optionA, optionB, optionC, optionD), และคำอธิบายเฉลย (explanation) ต้องเขียนเป็นภาษาไทย 100% เท่านั้น!**
3. ❌ **ห้ามออกข้อสอบเป็นภาษาอังกฤษเด็ดขาด!** ห้ามมีตัวเลือกภาษาอังกฤษ หรือคำตอบเป็นภาษาอังกฤษ (ยกเว้นตัวย่อกฎหมาย เช่น PDPA หรือคำทับศัพท์ที่มีคำภาษาไทยกำกับชัดเจนเท่านั้น)
...
[
  {
    "questionText": "สถานการณ์คดีจำลองหรือคำถามข้อกฎหมายที่เป็นภาษาไทย...",
    "optionA": "ตัวเลือก ก (ภาษาไทย)",
    "optionB": "ตัวเลือก ข (ภาษาไทย)",
    "optionC": "ตัวเลือก ค (ภาษาไทย)",
    "optionD": "ตัวเลือก ง (ภาษาไทย)",
    "correctOption": "A",
    "explanation": "คำอธิบายเฉลยอย่างละเอียดเป็นภาษาไทย อ้างอิงมาตราและเหตุผลทางกฎหมาย..."
  }
]`;
}
...
async function crossModelAuditExamQuestions({ questions, subject, subcategory, primaryEngine = '', customApiKey = '', groqApiKey = '', openrouterApiKey = '' }) {
  ...
  const isEnglishSubject = (subject || '').toLowerCase().includes('english') || (subject || '').toLowerCase().includes('อังกฤษ');
...
5. ⛔️ กฎเหล็กภาษาไทย 100%: ${isEnglishSubject ? 'โจทย์และตัวเลือกเป็นภาษาอังกฤษ แต่คำอธิบายสามารถใช้ไทยหรืออังกฤษได้' : 'ทั้งคำตอบและการวิเคราะห์ (reasoning) ต้องเขียนเป็นภาษาไทยเท่านั้น ห้ามตอบเป็นภาษาอังกฤษเด็ดขาด!'}
...
[
  {
    ...
    "hasFlaw": false,
    "flawDetails": null,
    "reasoning": "คำอธิบายเหตุผลสั้นๆ เป็นภาษาไทย"
  }
]`;
  ...
  const finalQuestions = questions.map((origQ, idx) => {
    ...
    if (isAgreement) {
      ...
      const isAuditorReasoningInvalid = isInvalidExplanationText(cleanAuditReasoning, isEnglishSubject);
      const isPrimaryExpInvalid = isInvalidExplanationText(cleanPrimaryExp, isEnglishSubject);
...
function isInvalidExplanationText(exp, isEnglishSubject = false) {
  if (!exp || typeof exp !== 'string') return true;
  const t = exp.trim();
  if (t.length < 5) return true;

  // Reject all-English explanation for non-English subjects (prevents LLM hallucinating in English)
  if (!isEnglishSubject) {
    const thaiCharCount = (t.match(/[\u0E00-\u0E7F]/g) || []).length;
    const latinCharCount = (t.match(/[a-zA-Z]/g) || []).length;
    if (thaiCharCount === 0 && latinCharCount > 15) {
      return true;
    }
  }
...
app.post('/api/admin/exams/preview-ai', authenticateToken, async (req, res) => {
  try {
    ...
    if (!contextText && (isSarabanSubject || knowledgeBase === 'สารบรรณ_๒๕๒๖' || docId === 'ALL_2526')) {
      ...
      try {
        const chMatch = (subcategory || '').match(/บทที่\s*(\d+)/);
        ...
        if (docs && docs.length > 0) {
          if (subcategory && subcategory !== 'ALL') {
            const matched = docs.filter(d => {
              ...
              if (chNum && (dt.includes(`บทที่ ${chNum}`) || dt.includes(`บทที่${chNum}`))) return true;
              if (cleanSub && (dt.includes(cleanSub) || d.content.toLowerCase().includes(cleanSub))) return true;
              return false;
            });
            if (matched.length > 0) {
          ...
          if (!contextText && subcategory === 'ALL') {
            contextText = docs.slice(0, 5).map(d => `[${d.title}]\n${d.content}`).join('\n\n');
          }
        }
          ...
          if (fs.existsSync(p)) {
            ...
            if (subcategory && subcategory !== 'ALL') {
              ...
              if (matched.length > 0) {
                ...
                contextText = raw.slice(0, 5).map(d => `[${d.title}]\n${d.content}`).join('\n\n');
              }
            } else {
              contextText = raw.slice(0, 5).map(d => `[${d.title}]\n${d.content}`).join('\n\n');
            }
...
app.post('/api/admin/exams/:examSetId/append-ai', authenticateToken, async (req, res) => {
  try {
    ...
    const chMatch = (examSet.subcategory || '').match(/บทที่\s*(\d+)/);
    const chNum = chMatch ? chMatch[1] : '';
    ...
    if (examSet.category === 'งานสารบรรณ' || cat.includes('สารบรรณ')) {
      ...
      try {
        ...
        if (docs && docs.length > 0) {
          ...
          if (matched.length > 0) contextText = matched.map(d => `[${d.title}]\n${d.content}`).join('\n\n');
        }
        if (!contextText) {
          const p = path.join(__dirname, 'data', 'computer_full.json');
          if (fs.existsSync(p)) {
        ...
        console.error('add-ai-questions comp err:', e);
      }
    } else if (isLaw) {
      try {
        const docs = await prisma.knowledgeDocument.findMany({ where: { category: { contains: 'กฎหมาย' } } });
        ...
        console.error('add-ai-questions law err:', e);
      }
    } else if (isThai) {
      const p = path.join(__dirname, 'data', 'thai_full.json');
      if (fs.existsSync(p)) {
